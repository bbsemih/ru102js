...
// formula to delete it in 30 days
//30 * 24 * 60 * 60;
...
const updateOptimized = async(meterReading) => {
    ...
    //Performt the compare-update operations using Lua script
    ...
    //evalsha evaluates the script from the server's cache by its SHA1 digest. these operations are performed atomically
    //server caches script by using SCRIPT LOAD command
    transaction.evalsha(compareAndUpdateScript.updateIfGreater, 1, key, 'maxWhGenerated', meterReading.whGenerated);
    transaction.evalsha(compareAndUpdateScript.updateIfLesser, 1, key, 'minWhGenerated', meterReading.whGenerated);
    transaction.evalsha(compareAndUpdateScript.updateIfGreater, 1, key, 'maxCapacity', meterReading.whGenerated - meterReading.whUsed);
